...
class Device:
    def __init__(self, sn: str, ip_mac: Union[Tuple[str, int], None] = None, sock: Union[socket.socket, None] = None, is_alive: bool = True):
        self.sn = sn
        self.ip_mac = ip_mac
        self.socket = sock
...
class GenericSwitcherDevice(Device):
    ...
    def __init__(self, sn: str, ip_mac: Union[str, None] = None, sock: Union[socket.socket, None] = None, is_alive: bool = True):
    ...
    def heartbeat(self) -> bool:
        ...
        for poll in range(3):
            ...
            try:
                ...
                if data:
                    logger.info('{}: device({}) heartbeat was succeed'.format(__file__, self.sn))
                    self.is_alive = True
                    return True
            except:
                err_msg = traceback.format_exc()
                logger.info('{}: device({}) heartbeat was fail'.format(__file__, self.sn))
                continue
        logger.warning('{}: device({}) heartbeat was fail'.format(__file__, self.sn))
        self.is_alive = False
        return False
